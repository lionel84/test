 ...
 * @LastEditTime: 2020-11-06 11:06:29
 * @Description: 
 */
...
void test(int count)
{
    ...
    nm_print_tree::tree_print<tree_node_t > tprint(
        ...
        [](tree_node_t* n){return n->value.second._score;},
    ...
    for(int i = 1; i<= test_size; i++){
        //auto it = rank_tree.insert(std::move(std::make_shared<rank_data_value_t>(i, rand(), rand()))); 
        auto it = rank_tree.insert(std::make_pair(i, rank_data_value_t(i, random(), random()))); 
   ...
   // tprint.print(rank_tree.m_root);
    ...
    printnow("根据排名查询节点");
    cout<<"节点大小: "<<size<<endl;
    cout<<"uid: "<<rank_tree.find_by_rank(3)->second->_uid<<" 积分: "<<rank_tree.find_by_rank(3)->second->_score<<endl;;
...
*/
    ...
    for(uint64_t i = 1; i<= test_size+100; i++){
        ...
        if(it != rank_tree.end())
        {
            auto it_v = *it;
            rank_tree.erase(it);
            it_v.second->_score = random();
            rank_tree.insert(it_v);
        }
        //cout<<"rank: "<<rank_tree.rank(it)<<" id: "<<it->second._uid<<" value: "<<it->second._score<<" ts: "<<it->second._ts<<endl;
    }
    printnow("根据uid修改积分");
